...
for power in $(seq 5 13); do
    echo "Running L1EM for insertion level 2^${power}..."

    bam_file="${output_dir}/star/GRCh38.p14.${record_name}.insert_level_${power}.pair.${fcov}x/Aligned.sortedByCoord.out.bam"
    output_prefix="${output_dir}/L1EM/GRCh38.p14.${record_name}.insert_level_${power}.pair.${fcov}x"
