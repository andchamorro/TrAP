...
echo "bowtie version: $(eval $bowtie --version)"
if [ ! -f "${reference%.*}.1.ebwt" ]; then
    echo "bowtie index not found. Creating index..."
    bowtie-build --threads "$threads" -f "$reference" "${reference%.*}"
    echo "bowtie index created."
fi

$bowtie --threads "$threads" -m 1 -S -y -v 3 -X 600 --chunkmbs 8184 "${reference%.*}" -1 "$pairedfile1" -2 "$pairedfile2" > "$outputdir/aligned_reads.sam"

# Step 2: Convert SAM to BAM
echo "[2/6] Converting SAM to BAM..."
samtools view -hbuS "$outputdir/aligned_reads.sam" > "$outputdir/aligned_reads.bam"

# Step 3: Sort BAM file
echo "[3/6] Sorting BAM file..."
samtools sort "$outputdir/aligned_reads.bam" -o "$outputdir/sorted_reads.bam"
