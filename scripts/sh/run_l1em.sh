...
# run_L1EM.sh quantifies against $L1EM_PATH/annotation/L1EM.400.{bed,fa}; install our BED there
# and rebuild the index (once) so full_counts.txt reports the synthetic UID loci, not empties.
source "$(dirname "${BASH_SOURCE[0]}")/_l1em_reference.sh"
ensure_l1em_reference "${L1EM_BED}" "${L1EM_PATH}" "${GENOME_FA}" l1em || exit 1
...
for power in ${POWERS}; do
  for dp in ${DELPROBS}; do
    ...
    # Resolve the BAM to an ABSOLUTE path BEFORE the `cd "${out}"` below — the relative ${bam}
    # would otherwise be realpath'd from the L1EM output dir (where it doesn't exist), passing
    # an empty BAM arg to run_L1EM.sh and producing an empty full_counts.txt.
    abs_bam="$(realpath "${bam}")"
    mkdir -p "${out}"
    echo "[l1em] ${base}: L1EM"
    ( cd "${out}" && bash "${abs_l1em}/run_L1EM.sh" "${abs_bam}" "${abs_l1em}" \
...
if [[ "${n_done}" -eq 0 && "${n_skip}" -eq 0 ]]; then
    ...
    echo "[l1em]   wrong dataset? for the background experiment pass SIM_MODEL=insert (→ synthetic/l1-host-insert.l1base)." >&2
    exit 1
fi
