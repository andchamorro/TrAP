...
if [ ! -f "${reference%.*}.1.ebwt" ]; then
    ...
    bowtie2-build --threads "$threads" "$reference" "${reference%.*}"
    echo "bowtie index created."
fi

$bowtie --threads "$threads" --very-sensitive -X 600 --no-mixed -x "${reference%.*}" -1 "$pairedfile1" -2 "$pairedfile2" -S "$outputdir/aligned_reads.sam"
