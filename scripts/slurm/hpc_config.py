    ...
    if isinstance(resources, dict):
        for stage_num, stage_res in resources.items():
            ...
            for res_key, res_suffix in (
                ...
                if val:
                    pairs.append(
                        ('SLURM_{0}_{1}'.format(res_suffix, stage_num), str(val))
                    )
...
def main(argv=None):
    ...
    ap.add_argument('--vars', metavar='FILE',
                    help='Emit shell KEY=VALUE assignments (skips env-set vars)')
    ap.add_argument('--run-vars', metavar='FILE',
                    help='Emit run-config KEY=VALUE assignments (skips env-set vars)')
    ap.add_argument('--check-cmds', nargs=2, metavar=('GROUP', 'FILE'),
                    help='Emit required command names for GROUP')
    args = ap.parse_args(argv)

    if args.vars:
        if not os.path.isfile(args.vars):
            print('[hpc_config] ERROR: config not found: {0}'.format(args.vars),
                  file=sys.stderr)
            return 1
        try:
            cfg = _load(args.vars)
        except Exception as exc:
            print('[hpc_config] ERROR: failed to parse {0}: {1}'.format(args.vars, exc),
                  file=sys.stderr)
            return 1
    ...
    if args.run_vars:
        if not os.path.isfile(args.run_vars):
            print('[hpc_config] ERROR: run config not found: {0}'.format(args.run_vars),
                  file=sys.stderr)
            return 1
        try:
            cfg = _load(args.run_vars)
        except Exception as exc:
            print('[hpc_config] ERROR: failed to parse {0}: {1}'.format(args.run_vars, exc),
                  file=sys.stderr)
            return 1
    ...
    if args.check_cmds:
        ...
        if not os.path.isfile(path):
            print('[hpc_config] ERROR: config not found: {0}'.format(path),
                  file=sys.stderr)
            return 1
        try:
            cfg = _load(path)
        ...
        if not cmds:
            print("[hpc_config] WARNING: no required_commands for group '{0}'".format(group),
                  file=sys.stderr)
        print(' '.join(cmds))
        return 0
